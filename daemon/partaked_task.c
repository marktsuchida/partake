...
static void continue_task_Open(struct partaked_handle *handle, void *data) {
    ...
    int status =
        partaked_channel_resume_open_object(d->conn->chan, handle, d->voucher);
    if (status != 0) {
...
int partaked_task_CreateVoucher(struct partaked_connection *conn,
    ...
    struct partaked_object *voucher;
    int status =
        partaked_channel_create_voucher(conn->chan, target_token, &voucher);
...
int partaked_task_Unknown(struct partaked_connection *conn,
    ...
    partaked_resparray_append_empty_response(
        resparr, req, partake_protocol_Status_INVALID_REQUEST);
